...
function createTickerItem(item) {
  ...
  const favicon = item.mode === "domains"
    ? `<img class="ticker-favicon" alt="" src="chrome://favicon/size/16@1x/https://${item.name}"/>`
    : "";
  el.innerHTML = `${favicon}<span class="ticker-name">${item.name}</span><span class="ticker-change ${item.trend}">${item.label === "NEW" ? "▲ NEW" : `${arrow} ${item.label}`}</span>`;
  return el;
}
