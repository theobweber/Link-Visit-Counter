...
function normalizeFocusSessionPresets(input) {
  ...
  DEFAULT_FOCUS_SESSION_PRESETS.forEach((preset) => byId.set(preset.id, { ...preset }));
  return Array.from(byId.values()).sort((a, b) => a.minutes - b.minutes);
}

function makePresetId(name, minutes) {
  const base = String(name || `${minutes} min`).toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "").slice(0, 28) || `preset_${minutes}`;
  return `${base}_${Date.now().toString(36).slice(-4)}`;
}

function saveFocusSessionPresets(presets) {
  return new Promise((resolve) => chrome.runtime.sendMessage({ type: "SAVE_FOCUS_SESSION_PRESETS", presets }, (response) => resolve(response || { ok: false })));
}

function setFocusSessionPreset(presetId) {
  return new Promise((resolve) => chrome.runtime.sendMessage({ type: "SET_FOCUS_SESSION_PRESET", presetId }, (response) => resolve(response || { ok: false })));
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const momentumTicker = document.getElementById("momentumTicker");
  ...
  renderMomentumTicker(appData, momentumTicker, currentScope);

  async function loadAndRender(tabName) {
    if (["daily", "weekly", "total"].includes(tabName)) currentScope = tabName;
...
function getActiveMomentumScope() {
  if (document.getElementById("weeklyTab")?.classList.contains("active")) return "weekly";
  if (document.getElementById("totalTab")?.classList.contains("active")) return "total";
  return "daily";
}
...
function buildMetricMap(appData, dateKeys, mode = "domains") {
  const map = {};
  dateKeys.forEach((key) => {
    const day = appData.daily?.[key];
    if (!day) return;
    Object.entries(day.domains || {}).forEach(([domain, stats]) => {
      const bucket = mode === "categories" ? (stats.category || "Other") : domain;
      map[bucket] = (map[bucket] || 0) + Number(stats.time || 0);
    });
  });
  return map;
}

function splitLifetimeKeys(appData) {
  const keys = Object.keys(appData.daily || {}).sort();
  if (keys.length < 2) return [[], []];
  const half = Math.max(1, Math.floor(keys.length / 2));
  return [keys.slice(0, half), keys.slice(half)];
}

function getTickerPeriodKeys(appData, scope) {
  if (scope === "daily") {
    return [[getTodayKey()], getDateRange(1, 1)];
  }
  if (scope === "weekly") {
    return [getDateRange(7), getDateRange(7, 7)];
  }
  return splitLifetimeKeys(appData);
}

function buildTickerItems(appData, scope = "daily") {
  const mode = appData.settings?.tickerMode === "categories" ? "categories" : "domains";
  const [currentKeys, previousKeys] = getTickerPeriodKeys(appData, scope);
  if (!currentKeys.length || !previousKeys.length) return [];

  const currentMap = buildMetricMap(appData, currentKeys, mode);
  const previousMap = buildMetricMap(appData, previousKeys, mode);
  const names = new Set([...Object.keys(currentMap), ...Object.keys(previousMap)]);

  const rows = [];
  names.forEach((name) => {
    const current = Number(currentMap[name] || 0);
    const previous = Number(previousMap[name] || 0);
    if (!current && !previous) return;

    if (!previous && current > 0) {
      rows.push({ name, mode, trend: "up", label: "NEW", pct: null, sortScore: 1000, current, previous });
      return;
    }
...
function createTickerItem(item) {
  ...
  const badge = item.mode === "domains"
    ? `<span class="ticker-favicon" aria-hidden="true">${String(item.name || "?").charAt(0).toUpperCase()}</span>`
    : "";
  el.innerHTML = `${badge}<span class="ticker-name">${item.name}</span><span class="ticker-change ${item.trend}">${item.label === "NEW" ? "▲ NEW" : `${arrow} ${item.label}`}</span>`;
  return el;
}

function renderMomentumTicker(appData, root, scope = "daily") {
  if (!root) return;
  const items = buildTickerItems(appData, scope);
  root.classList.remove("empty");
  root.innerHTML = "";

  if (!items.length) {
    root.classList.add("empty");
    root.textContent = "Not enough data for trend analysis.";
    return;
  }
  ...
  firstLoop.className = "ticker-loop";
  items.forEach((item) => firstLoop.appendChild(createTickerItem(item)));
  track.appendChild(firstLoop);
  root.appendChild(track);

  const shouldAnimate = firstLoop.scrollWidth > root.clientWidth;
  if (shouldAnimate) {
    const secondLoop = firstLoop.cloneNode(true);
    secondLoop.setAttribute("aria-hidden", "true");
    track.appendChild(secondLoop);
...
function renderFocusWidget(appData, content) {
  const focus = appData.focusSession || {};
  const presets = normalizeFocusSessionPresets(appData.focusSessionPresets);
  ...
  const selectedPresetId = presets.find((preset) => preset.id === focus.selectedPresetId)?.id || presets[0]?.id || "default_25";

  const c = card(t("focusSession"), [t("contextSwitches", [focus.tabSwitches || 0]), t("distractingSwitches", [focus.distractingSwitches || 0])]);
  const row = document.createElement("div");
  row.className = "setting-row";
  row.innerHTML = `<div class="small" style="flex:1">${t("focusSessionState", [state])}</div><select id="focusPresetSelect" style="flex:1"></select><button id="startFocusBtn" class="action">${t("start")}</button><button id="stopFocusBtn" class="ghost">${t("stop")}</button>`;
  c.appendChild(row);

  const presetForm = document.createElement("div");
  presetForm.className = "focus-preset-form";
  presetForm.style.display = "none";
  presetForm.innerHTML = `
    <div class="setting-row"><input id="focusPresetName" placeholder="Session Name (optional)" style="flex:1;"/></div>
    <div class="setting-row"><span class="small">Minutes</span><button class="ghost focus-adjust" data-delta="-5" type="button">-5</button><button class="ghost focus-adjust" data-delta="-1" type="button">-1</button><input id="focusPresetMinutes" type="number" min="1" max="480" value="25" style="width:72px;"/><button class="ghost focus-adjust" data-delta="1" type="button">+1</button><button class="ghost focus-adjust" data-delta="5" type="button">+5</button></div>
    <div class="setting-row"><button id="saveFocusPresetBtn" class="action">Save</button><button id="cancelFocusPresetBtn" class="ghost">Cancel</button></div>
  `;
  c.appendChild(presetForm);

  const customList = document.createElement("div");
  customList.className = "small";
  c.appendChild(customList);

  const presetSelect = c.querySelector("#focusPresetSelect");
  let editPresetId = null;

  const renderPresetSelect = () => {
    presetSelect.innerHTML = "";
    normalizeFocusSessionPresets(appData.focusSessionPresets).forEach((preset) => {
    ...
    addOption.textContent = "+ Add Custom Session";
    presetSelect.appendChild(addOption);
    presetSelect.value = selectedPresetId;
  };

  const renderCustomList = () => {
    const customPresets = normalizeFocusSessionPresets(appData.focusSessionPresets).filter((preset) => !preset.isDefault);
    ...
    customPresets.forEach((preset) => {
      ...
      rowEl.innerHTML = `<span style="flex:1;">${preset.name} — ${preset.minutes} min</span><button class="ghost" data-edit-preset="${preset.id}">Edit</button><button class="ghost" data-delete-preset="${preset.id}">Delete</button>`;
      customList.appendChild(rowEl);
    });
  };

  const openPresetForm = (preset = null) => {
    editPresetId = preset?.id || null;
    c.querySelector("#focusPresetName").value = preset?.name || "";
    c.querySelector("#focusPresetMinutes").value = String(preset?.minutes || 25);
  ...
  presetSelect.addEventListener("change", async () => {
    ...
    await setFocusSessionPreset(presetSelect.value);
  });

  c.querySelectorAll(".focus-adjust").forEach((btn) => {
    btn.addEventListener("click", () => {
      const input = c.querySelector("#focusPresetMinutes");
      const next = Math.max(1, Math.min(480, Number(input.value || 0) + Number(btn.dataset.delta || 0)));
      input.value = String(next);
    });
  });

  c.querySelector("#cancelFocusPresetBtn").addEventListener("click", closePresetForm);

  c.querySelector("#saveFocusPresetBtn").addEventListener("click", async () => {
    const minutes = Math.max(1, Math.min(480, Math.round(Number(c.querySelector("#focusPresetMinutes").value || 0))));
    if (!minutes) return;
    const nameInput = c.querySelector("#focusPresetName").value.trim();
    const name = nameInput || `${minutes} min`;

    let nextPresets = normalizeFocusSessionPresets(appData.focusSessionPresets).filter((preset) => !preset.isDefault);
    if (editPresetId) nextPresets = nextPresets.filter((preset) => preset.id !== editPresetId);
    nextPresets.push({ id: editPresetId || makePresetId(name, minutes), name, minutes, isDefault: false });
    nextPresets = [...DEFAULT_FOCUS_SESSION_PRESETS, ...nextPresets].sort((a, b) => a.minutes - b.minutes);

    const res = await saveFocusSessionPresets(nextPresets);
    if (res?.ok) {
      appData.focusSessionPresets = res.presets;
      if (res.selectedPresetId) appData.focusSession.selectedPresetId = res.selectedPresetId;
      renderDaily(appData, content);
    }
  });

  customList.addEventListener("click", async (event) => {
    const editBtn = event.target.closest("button[data-edit-preset]");
    const deleteBtn = event.target.closest("button[data-delete-preset]");

    if (editBtn) {
      const preset = normalizeFocusSessionPresets(appData.focusSessionPresets).find((item) => item.id === editBtn.dataset.editPreset);
      if (preset) openPresetForm(preset);
      return;
    }

    if (deleteBtn) {
      if (!confirm("Delete this session preset?")) return;
      const removeId = deleteBtn.dataset.deletePreset;
      const kept = normalizeFocusSessionPresets(appData.focusSessionPresets).filter((preset) => preset.isDefault || preset.id !== removeId);
      const res = await saveFocusSessionPresets(kept);
      if (res?.ok) {
        appData.focusSessionPresets = res.presets;
        appData.focusSession.selectedPresetId = res.selectedPresetId || appData.focusSession.selectedPresetId;
        renderDaily(appData, content);
      }
    }
  });

  c.querySelector("#startFocusBtn").addEventListener("click", () => {
    const activeId = presetSelect.value === "__add_custom__" ? selectedPresetId : presetSelect.value;
    const preset = normalizeFocusSessionPresets(appData.focusSessionPresets).find((item) => item.id === activeId) || normalizeFocusSessionPresets(appData.focusSessionPresets)[0];
    const minutes = Number(preset?.minutes || 25);
    chrome.runtime.sendMessage({ type: "START_FOCUS_SESSION", durationMinutes: minutes, presetId: preset?.id });
  });
...
function renderSettings(appData, content, onSave) {
  ...
  content.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="setting-row"><span style="flex:1" class="small">Ticker Display Mode</span><label class="small"><input type="radio" name="tickerMode" value="domains" ${(settings.tickerMode || "domains") === "domains" ? "checked" : ""}/> Domains</label><label class="small"><input type="radio" name="tickerMode" value="categories" ${(settings.tickerMode || "domains") === "categories" ? "checked" : ""}/> Categories</label></div>
  ...
  content.querySelectorAll('input[name="tickerMode"]').forEach((radio) => {
    radio.addEventListener("change", async (event) => {
      const tickerMode = event.target.value === "categories" ? "categories" : "domains";
      const nextSettings = { ...settings, tickerMode };
      await persistSettings(nextSettings);
      const fresh = await fetchData();
      const tickerRoot = document.getElementById("momentumTicker");
      renderMomentumTicker(fresh, tickerRoot, getActiveMomentumScope());
    });
  });
