...
function analyzeTransitionsForDay(day) {
  ...
  if (toDistracting.length >= 3) {
    ...
    if (afternoonHits >= Math.max(2, Math.floor(toDistracting.length / 2))) {
      insights.push("You frequently switch from Work to Social Media in the afternoon.");
    } else {
      insights.push("You often switch from productive work to distracting categories within 3 minutes.");
    }
  }
  ...
  if (longFocusToDistracting.length >= 2) {
    insights.push("Most distractions happen after 40+ minute focus sessions.");
  }
...
function applyMicroIntervention(categoryType, settings) {
  ...
  if (categoryType === "distracting" && continuousCategoryStart.categoryType === "distracting") {
    ...
    if (elapsed >= DISTRACTION_INTERVENTION_MS && now - lastInterventionAt.distracting >= INTERVENTION_COOLDOWN_MS) {
      ...
      maybeNotify("micro-distracting", "Micro Intervention", "Consider a short break?", settings.notificationsEnabled);
    }
  }
  ...
  if (categoryType === "productive" && continuousCategoryStart.categoryType === "productive") {
    ...
    if (elapsed >= PRODUCTIVE_INTERVENTION_MS && now - lastInterventionAt.productive >= INTERVENTION_COOLDOWN_MS) {
      ...
      maybeNotify("micro-productive", "Great deep work session", "You have sustained productive focus for over 60 minutes.", settings.notificationsEnabled);
    }
  }
}
...
function runWeeklyAnalysis(data) {
  ...
  if (driftHour !== null) {
    ...
    data.analyticsCache.energyDrift = `Your productivity tends to decrease after ${h12} ${ampm}.`;
  }
}
    ...
    if (data.focusSession.active) {
      ...
      if (type === "distracting") {
        ...
        maybeNotify(
          "focus-distracting",
          "Focus session alert",
          `You switched to ${hostname} (${toCategory}).`,
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "GET_APP_DATA") {
    withData((data) => {
      ...
      saveData(data, () => sendResponse({ ok: true, data }));
    });
    return true;
  }
