...
function renderCommandCenter(appData, root) {
  ...
  const focusSession = appData.focusSession || {};
  const active = Boolean(focusSession.active && focusSession.endTime);
  const remaining = active ? Math.max(0, focusSession.endTime - Date.now()) : 0;

  root.classList.toggle("active-focus", active);
  root.innerHTML = `
    <div class="metric-top">
      <div class="metric-main">${focusScore}%</div>
      <div class="metric-sub">Focus Score</div>
    </div>
    <div class="metric-row">Browser Balance: <strong>${balance}/100</strong></div>
    <div class="metric-row">🔥 ${streak}-Day Focus Streak</div>
    <div class="progress-track"><div class="progress-fill" style="width:${Math.max(0, Math.min(100, focusScore))}%"></div></div>
    <div class="thin-divider"></div>
    <div class="metric-row">Focus Mode: <span id="focusState" ${active ? `data-end-time="${focusSession.endTime}"` : ""}>${
      active ? `Active • ${formatDuration(remaining)} left` : "Inactive"
    }</span></div>
  `;
}
...
function renderTopDomainsBar(domains, content) {
  const c = card("Top 5 Domains Today");
...
function renderCategoryPie(categoryTotals, content) {
  const c = card("Category Time Distribution");
  ...
  segments.forEach((s) => {
    const row = document.createElement("div");
    row.className = "small";
    row.innerHTML = `<span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${s.color};margin-right:6px;"></span>${
      s.name
    }: ${Math.round(s.pct)}%`;
    c.appendChild(row);
  });
...
function renderHeatmap(day, content) {
  const c = card("Context Switching Heatmap");
  ...
  for (let h = 0; h < 24; h += 1) {
    const bucket = day.hourly?.[String(h)] || { focusScore: 0, switches: 0 };
    ...
    const p = Math.max(0, Math.min(100, bucket.focusScore || 0));
    ...
    cell.title = `${h}:00 • focus ${bucket.focusScore || 0} • switches ${bucket.switches || 0}`;
...
function renderFocusWidget(appData, content) {
  ...
  const c = card("Focus Session", [`Context Switches: ${focus.tabSwitches || 0} • Distracting switches: ${focus.distractingSwitches || 0}`]);
  const row = document.createElement("div");
  row.className = "setting-row";
  row.innerHTML = `
    <div class="small" style="flex:1">State: ${state}</div>
  ...
  c.querySelector("#startFocusBtn").addEventListener("click", () => {
    chrome.runtime.sendMessage({ type: "START_FOCUS_SESSION", durationMinutes: Number(c.querySelector("#focusDuration").value) });
  });
...
function renderRatingPrompt(appData, content) {
  ...
  const c = card("Session Quality Rating", ["How focused were you? (1–5)"]);
  const row = document.createElement("div");
  row.className = "setting-row";
  row.innerHTML = `
    ...
    <button id="saveRatingBtn" class="action">Save</button>
  `;
  c.appendChild(row);
...
function renderDaily(appData, content) {
  ...
  content.appendChild(
    card("Daily Metrics", [
      ...
      `Browser Balance: ${day.dailyScores?.browserBalance || 0}/100`,
      `Longest Focus Session: ${formatDuration(stats.longestFocusSessionMs)}`,
      `Context Switches: ${stats.contextSwitches}`,
      `Average Recovery Time: ${formatDuration(day.dailyScores?.averageRecoveryMs || 0)}`
    ])
  );
  ...
  if (insightLines.length) {
    content.appendChild(card("🧠 Today’s Insight", insightLines, "insight-card"));
  }

  const shortSessionInsights = [];
  Object.entries(day.shortVisits || {}).forEach(([domain, count]) => {
    if (count >= 8) shortSessionInsights.push(`Frequent Short Sessions detected on ${domain} (${count} short sessions today).`);
  });
  if (shortSessionInsights.length) {
    content.appendChild(card("Frequent Short Sessions", shortSessionInsights));
  }
...
function renderWeekly(appData, content) {
  ...
  content.appendChild(
    card("Weekly Metrics", [
      `Focus Score: ${nowStats.focusScore}/100 (${pctChange(nowStats.focusScore, prevStats.focusScore)})`,
      `Longest Focus Session: ${formatDuration(nowStats.longestFocusSessionMs)}`,
      `Context Switches: ${nowStats.contextSwitches}`
    ])
  );

  const reflection = [];
  if (summary.mostVisitedDomain) reflection.push(`Most visited domain: ${summary.mostVisitedDomain} (${summary.mostVisitedCount || 0} visits)`);
  if (summary.mostProductiveDay) reflection.push(`Most productive day: ${summary.mostProductiveDay}`);
  if (summary.mostDistractedHour !== undefined && summary.mostDistractedHour !== "") reflection.push(`Most distraction-heavy hour: ${summary.mostDistractedHour}:00`);
  if (summary.focusScoreTrend) reflection.push(`Focus trend vs previous week: ${summary.focusScoreTrend}`);
  if (summary.timeReclaimedMs > 0) reflection.push(`You reclaimed ${formatDuration(summary.timeReclaimedMs)} compared to last week.`);
  if (summary.distractingIncreaseMs > 0) reflection.push(`Distracting time increased by ${formatDuration(summary.distractingIncreaseMs)} compared to last week.`);
  if (summary.avgRecoveryMs > 0) reflection.push(`Average recovery time after distraction: ${formatDuration(summary.avgRecoveryMs)}.`);
  if (appData.analyticsCache?.energyDrift) reflection.push(appData.analyticsCache.energyDrift);
  if (reflection.length) content.appendChild(card("Weekly Reflection Summary", reflection));

  const dc = card("Domain Changes (7-day)");
  Object.entries(nowStats.domainTotals)
    ...
    .forEach(([domain, curr]) => {
      ...
      row.innerHTML = `<div class="site-top"><span class="site-name">${domain}</span><span class="site-stats">${curr.visits} visits (${pctChange(
        curr.visits,
        prev.visits
      )}) • ${formatDuration(curr.time)} (${pctChange(curr.time, prev.time)})</span></div>`;
      dc.appendChild(row);
    });
  content.appendChild(dc);

  const cc = card("Category Changes (7-day)");
  Object.entries(nowStats.categoryTotals)
    .sort((a, b) => b[1].time - a[1].time)
    .forEach(([category, curr]) => {
      const prev = prevStats.categoryTotals[category] || { time: 0, visits: 0 };
      const row = document.createElement("div");
      row.className = "site-entry";
      row.innerHTML = `<div class="site-top"><span class="site-name">${category}</span><span class="site-stats">${curr.visits} visits (${pctChange(
        curr.visits,
        prev.visits
      )}) • ${formatDuration(curr.time)} (${pctChange(curr.time, prev.time)})</span></div>`;
      cc.appendChild(row);
...
function renderSettings(appData, content, onSave) {
  ...
  content.innerHTML = `
    ...
    <div class="card">
      <div class="card-title">Domain Assignment</div>
    ...
    <div class="card">
      ...
      <div class="setting-row"><span style="flex:1" class="small">Notifications</span><input type="checkbox" class="toggle" id="notifToggle" ${
        settings.notificationsEnabled === false ? "" : "checked"
      }/></div>
      <div class="setting-row"><span style="flex:1" class="small">Micro Interventions</span><input type="checkbox" class="toggle" id="microToggle" ${
        settings.microInterventionsEnabled === false ? "" : "checked"
      }/></div>
      <div class="setting-row"><input id="focusThresholdInput" type="number" min="1" max="100" value="${
        Number(settings.focusScoreProductiveThreshold || 70)
      }" style="flex:1;"/><button id="saveThresholdBtn" class="action">Save threshold</button></div>
    </div>

    <div class="card">
      <div class="card-title">Data Management</div>
      <div class="setting-row"><button id="exportBtn" class="ghost" style="flex:1;">Export JSON</button><label for="importInput" class="ghost" style="text-align:center;flex:1;padding:8px;border-radius:10px;cursor:pointer;">Import JSON</label><input id="importInput" type="file" accept="application/json" style="display:none;"/></div>
